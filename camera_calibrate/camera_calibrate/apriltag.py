...
def _get_dll_path():
    ...
    return [
        ...
        "/root/ros2_ws/src/open_manipulator/docker/workspace/ros2_ws/src/AprilTag/build/lib"
    ]
...
def detect_tags_filterred(image,
                detector,
                camera_params=(3156.71852, 3129.52243, 359.097908, 239.736909),
                tag_size=0.0762,
                vizualization=0,
                ...
                annotation=False,
                th_hamming_code = 0,
                th_init_pose_err = 10000,
                th_final_pose_err = 0.01,
                th_decision_margin = 60
               ):
    ...
    for i, detection in enumerate(detections):
        if detection.hamming <= th_hamming_code and detection.decision_margin >= th_decision_margin:
            if verbose==2 or verbose==3:
                print( 'Detection {} of {}:'.format(i+1, num_detections))
                print()
                print(detection.tostring(indent=2))
            ...
            pose, e0, e1 = detector.detection_pose(detection, camera_params, tag_size)
            if e0 < th_init_pose_err and e1 < th_final_pose_err:
                if vizualization==1:
                    _draw_pose_box(overlay, camera_params, tag_size, pose)
                elif vizualization==2:
                    _draw_pose_axes(overlay, camera_params, tag_size, pose, detection.center)
