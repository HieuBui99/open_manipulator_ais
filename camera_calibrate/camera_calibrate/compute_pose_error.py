...
def transformation_error(T1, T2):
    ...
    trans_error = np.linalg.norm(t1[:2] - t2[:2])  # Only consider x, y translation

    # Rotation error: angle of relative rotation
    R_rel = R1.T @ R2
    rotvec = R.from_matrix(R_rel).as_rotvec()
    rot_error_deg = np.linalg.norm(rotvec) * 180.0 / np.pi
...
def apriltag_image(images,
    ...
    for s, img in zip(serials, images):
        ...
        for r in result:
            r['serial'] = s
        pose = extrinsics_matrices[s].dot(result[0]['pose'])
        poses.append(pose)
        overlays.append(overlay)
    print("errors:{}".format(transformation_error(*poses)))
    if display_images:
...
if __name__ == "__main__":
    ...
    #     "826212070364": T_cam2,
    # }
    extrinsics_matrices,camera_matrices = get_camera_tf()
    ########################################################################
